...
const DepositScreen = ({ navigation }) => {
  ...
  const deposit = async () => {
    ...
    const response = await homeApi.post("/Deposit?accountNumber=" + accountNumber + "&amount=" + amount, null, { headers: { Authorization: "Bearer " + userToken } });
    setProgress(false);
  };
  ...
  return (
    isProgress ?
      ...
      <View style={{ flex: 1, backgroundColor: '#ffffff', padding: 20 }}>
        ...
        <TouchableOpacity onPress={() => deposit()} style={{ alignItems: "center", marginTop: 60 }}>
          <Text style={styles.depositButton}>YATIR</Text>
        </TouchableOpacity>
      </View>
  );
};
